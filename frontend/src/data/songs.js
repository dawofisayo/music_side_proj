export const SONG_DATABASE = [
    // Pop - USA
    {
      id: 1,
      title: "Anti-Hero",
      artist: "Taylor Swift",
      album: "Midnights",
      spotifyId: "0V3wPSX9ygBnCm8psDIegu",
      youtubeId: "b1kbLwvqugk",
      releaseYear: 2022,
      country: "USA",
      genre: "Pop"
    },
    {
      id: 2,
      title: "Bad Guy",
      artist: "Billie Eilish",
      album: "WHEN WE ALL FALL ASLEEP",
      spotifyId: "2Fxmhks0bxGSBdJ92vM42m",
      youtubeId: "DyDfgMOUjCI",
      releaseYear: 2019,
      country: "USA",
      genre: "Pop"
    },
    {
      id: 3,
      title: "Flowers",
      artist: "Miley Cyrus",
      album: "Endless Summer Vacation",
      spotifyId: "0yLdNVWF3Srea0uzk55zFn",
      youtubeId: "G7KNmW9a75Y",
      releaseYear: 2023,
      country: "USA",
      genre: "Pop"
    },
    
    // Pop - UK
    {
      id: 4,
      title: "Levitating",
      artist: "Dua Lipa",
      album: "Future Nostalgia",
      spotifyId: "39LLxExYz6ewLAcYrzQQyP",
      youtubeId: "TUVcZfQe-Kw",
      releaseYear: 2020,
      country: "UK",
      genre: "Pop"
    },
    {
      id: 5,
      title: "Shape of You",
      artist: "Ed Sheeran",
      album: "÷",
      spotifyId: "7qiZfU4dY1lWllzX7mPBI0",
      youtubeId: "JGwWNGJdvx8",
      releaseYear: 2017,
      country: "UK",
      genre: "Pop"
    },
    {
      id: 6,
      title: "Watermelon Sugar",
      artist: "Harry Styles",
      album: "Fine Line",
      spotifyId: "6UelLqGlWMcVH1E5c4H7lY",
      youtubeId: "E07s5ZYygMg",
      releaseYear: 2019,
      country: "UK",
      genre: "Pop"
    },
  
    // Pop - Canada
    {
      id: 7,
      title: "Blinding Lights",
      artist: "The Weeknd",
      album: "After Hours",
      spotifyId: "0VjIjW4GlUZAMYd2vXMi3b",
      youtubeId: "4NRXx6U8ABQ",
      releaseYear: 2019,
      country: "Canada",
      genre: "Pop"
    },
    {
      id: 8,
      title: "Call Me Maybe",
      artist: "Carly Rae Jepsen",
      album: "Kiss",
      spotifyId: "20I6sIOMTCkB6w7ryavxtO",
      youtubeId: "fWNaR-rxAic",
      releaseYear: 2011,
      country: "Canada",
      genre: "Pop"
    },
  
    // Pop - Australia
    {
      id: 9,
      title: "Dance Monkey",
      artist: "Tones and I",
      album: "The Kids Are Coming",
      spotifyId: "2XU0oxnq2qxCpomAAuJY8K",
      youtubeId: "q0hyYWKXF0Q",
      releaseYear: 2019,
      country: "Australia",
      genre: "Pop"
    },
  
    // Hip Hop - USA
    {
      id: 10,
      title: "HUMBLE.",
      artist: "Kendrick Lamar",
      album: "DAMN.",
      spotifyId: "7KXjTSCq5nL1LoYtL7XAwS",
      youtubeId: "tvTRZJ-4EyI",
      releaseYear: 2017,
      country: "USA",
      genre: "Hip Hop"
    },
    {
      id: 11,
      title: "Sicko Mode",
      artist: "Travis Scott",
      album: "ASTROWORLD",
      spotifyId: "2xLMifQCjDGFmkHkpNLD9h",
      youtubeId: "6ONRf7h3Mdk",
      releaseYear: 2018,
      country: "USA",
      genre: "Hip Hop"
    },
    {
      id: 12,
      title: "Lose Yourself",
      artist: "Eminem",
      album: "8 Mile",
      spotifyId: "5Z01UMMf7V1o0MzF86s6WJ",
      youtubeId: "_Yhyp-_hX2s",
      releaseYear: 2002,
      country: "USA",
      genre: "Hip Hop"
    },
  
    // Hip Hop - Canada
    {
      id: 13,
      title: "Hotline Bling",
      artist: "Drake",
      album: "Views",
      spotifyId: "0wwPcA6wtMf6HUMpIRdeP7",
      youtubeId: "uxpDa-c-4Mc",
      releaseYear: 2015,
      country: "Canada",
      genre: "Hip Hop"
    },
    {
      id: 14,
      title: "One Dance",
      artist: "Drake",
      album: "Views",
      spotifyId: "12VWzyLqIg2RK7wM7HiUQN",
      youtubeId: "eILJja7dFLY",
      releaseYear: 2016,
      country: "Canada",
      genre: "Hip Hop"
    },
  
    // R&B - USA
    {
      id: 15,
      title: "Crazy in Love",
      artist: "Beyoncé",
      album: "Dangerously In Love",
      spotifyId: "5IVuqXILoxVWvWEPm82Jxr",
      youtubeId: "ViwtNLUqkMY",
      releaseYear: 2003,
      country: "USA",
      genre: "R&B"
    },
    {
      id: 16,
      title: "Redbone",
      artist: "Childish Gambino",
      album: "Awaken, My Love!",
      spotifyId: "0wXuerDYiBnERgIpbb3JBR",
      youtubeId: "Kp7eSUU9oy8",
      releaseYear: 2016,
      country: "USA",
      genre: "R&B"
    },
  
    // Rock - UK
    {
      id: 17,
      title: "Bohemian Rhapsody",
      artist: "Queen",
      album: "A Night at the Opera",
      spotifyId: "4u7EnebtmKWzUH433cf5Qv",
      youtubeId: "fJ9rUzIMcZQ",
      releaseYear: 1975,
      country: "UK",
      genre: "Rock"
    },
    {
      id: 18,
      title: "Mr. Brightside",
      artist: "The Killers",
      album: "Hot Fuss",
      spotifyId: "003vvx7Niy0yvhvHt4a68B",
      youtubeId: "gGdGFtwCNBE",
      releaseYear: 2003,
      country: "USA",
      genre: "Rock"
    },
    {
      id: 19,
      title: "Don't Stop Believin'",
      artist: "Journey",
      album: "Escape",
      spotifyId: "4bHsxqR3GMrXTxEPLuK5ue",
      youtubeId: "1k8craCGpgs",
      releaseYear: 1981,
      country: "USA",
      genre: "Rock"
    },
  
    // Country - USA
    {
      id: 20,
      title: "Tennessee Whiskey",
      artist: "Chris Stapleton",
      album: "Traveller",
      spotifyId: "4VqN98zKgHVwAXRF8WHtzq",
      youtubeId: "4zAThXFOy2c",
      releaseYear: 2015,
      country: "USA",
      genre: "Country"
    },
    {
      id: 21,
      title: "Jolene",
      artist: "Dolly Parton",
      album: "Jolene",
      spotifyId: "3S9p6RoBkkwxHMNB5oDPXW",
      youtubeId: "Ixrje2rXLMA",
      releaseYear: 1973,
      country: "USA",
      genre: "Country"
    },
  
    // EDM - Sweden
    {
      id: 22,
      title: "Wake Me Up",
      artist: "Avicii",
      album: "True",
      spotifyId: "0nrRP2bk19rLc0orkWPQk2",
      youtubeId: "IcrbM1l_BoI",
      releaseYear: 2013,
      country: "Sweden",
      genre: "EDM"
    },
    {
      id: 23,
      title: "Don't You Worry Child",
      artist: "Swedish House Mafia",
      album: "Until Now",
      spotifyId: "4HNrF3p7ngEWP8qX0N3Bnn",
      youtubeId: "1y6smkh6c-0",
      releaseYear: 2012,
      country: "Sweden",
      genre: "EDM"
    },
  
    // EDM - Netherlands
    {
      id: 24,
      title: "Animals",
      artist: "Martin Garrix",
      album: "Animals",
      spotifyId: "4LeOVWB169Qp6m3xJfFWFT",
      youtubeId: "gCYcHz2k5x0",
      releaseYear: 2013,
      country: "Netherlands",
      genre: "EDM"
    },
  
    // K-Pop - South Korea
    {
      id: 25,
      title: "Dynamite",
      artist: "BTS",
      album: "BE",
      spotifyId: "5QDLhrAOJJdNAmCTJ8xMyW",
      youtubeId: "gdZLi9oWNZg",
      releaseYear: 2020,
      country: "South Korea",
      genre: "K-Pop"
    },
    {
      id: 26,
      title: "Gangnam Style",
      artist: "PSY",
      album: "PSY 6 (Six Rules), Part 1",
      spotifyId: "0pXYTT5dPeSp3c1u9mpPij",
      youtubeId: "9bZkp7q19f0",
      releaseYear: 2012,
      country: "South Korea",
      genre: "K-Pop"
    },
    {
      id: 27,
      title: "Kill This Love",
      artist: "BLACKPINK",
      album: "Kill This Love",
      spotifyId: "6hvFrHNcFCgjAkMrngjcP4",
      youtubeId: "2S24-y0Ij3Y",
      releaseYear: 2019,
      country: "South Korea",
      genre: "K-Pop"
    },
  
    // Reggae/Dancehall - Jamaica
    {
      id: 28,
      title: "One Love",
      artist: "Bob Marley",
      album: "Exodus",
      spotifyId: "52iwsT98xCoGgiGchL2LkU",
      youtubeId: "vdB-8eLEW8g",
      releaseYear: 1977,
      country: "Jamaica",
      genre: "Reggae"
    },
    {
      id: 29,
      title: "Temperature",
      artist: "Sean Paul",
      album: "The Trinity",
      spotifyId: "2VixOjvFDD5EhXKZGE8Fxm",
      youtubeId: "dW2MmuA1nI4",
      releaseYear: 2005,
      country: "Jamaica",
      genre: "Dancehall"
    },
  
    // Latin - Colombia
    {
      id: 30,
      title: "Hips Don't Lie",
      artist: "Shakira",
      album: "Oral Fixation, Vol. 2",
      spotifyId: "4aVuWgvD0X63hcOCnZtNFA",
      youtubeId: "DUT5rEU6pqM",
      releaseYear: 2006,
      country: "Colombia",
      genre: "Latin Pop"
    },
  
    // Latin - Puerto Rico
    {
      id: 31,
      title: "Despacito",
      artist: "Luis Fonsi",
      album: "Vida",
      spotifyId: "6habFhsOp2NvshLv26DqMb",
      youtubeId: "kJQP7kiw5Fk",
      releaseYear: 2017,
      country: "Puerto Rico",
      genre: "Reggaeton"
    },
    {
      id: 32,
      title: "Dákiti",
      artist: "Bad Bunny",
      album: "El Último Tour Del Mundo",
      spotifyId: "3HP9TrlMRH3RkHxP1qPCNI",
      youtubeId: "TmKh7lAwnBI",
      releaseYear: 2020,
      country: "Puerto Rico",
      genre: "Reggaeton"
    },
  
    // Afrobeats - Nigeria
    {
      id: 33,
      title: "Essence",
      artist: "Wizkid",
      album: "Made in Lagos",
      spotifyId: "5U5TNxHitSUk22bMVBBzNW",
      youtubeId: "YfA_RPgjRoo",
      releaseYear: 2020,
      country: "Nigeria",
      genre: "Afrobeats"
    },
    {
      id: 34,
      title: "Last Last",
      artist: "Burna Boy",
      album: "Love, Damini",
      spotifyId: "5QL0xKS9UgbycvAHqF8ePd",
      youtubeId: "4IkPQZAM4x0",
      releaseYear: 2022,
      country: "Nigeria",
      genre: "Afrobeats"
    },
  
    // Alternative - USA
    {
      id: 35,
      title: "Smells Like Teen Spirit",
      artist: "Nirvana",
      album: "Nevermind",
      spotifyId: "4CeeEOM32jQcH3eN9Q2dGj",
      youtubeId: "hTWKbfoikeg",
      releaseYear: 1991,
      country: "USA",
      genre: "Alternative Rock"
    },
    {
      id: 36,
      title: "Creep",
      artist: "Radiohead",
      album: "Pablo Honey",
      spotifyId: "70LcF31zb1H0PyJoS1Sx1r",
      youtubeId: "XFkzRNyygfk",
      releaseYear: 1992,
      country: "UK",
      genre: "Alternative Rock"
    },
  
    // Indie - Australia
    {
      id: 37,
      title: "Somebody That I Used to Know",
      artist: "Gotye",
      album: "Making Mirrors",
      spotifyId: "4qiyAPHALHFp5YqXTOzE1U",
      youtubeId: "8UVNT4wvIGY",
      releaseYear: 2011,
      country: "Australia",
      genre: "Indie Pop"
    },
  
    // Electronic - France
    {
      id: 38,
      title: "One More Time",
      artist: "Daft Punk",
      album: "Discovery",
      spotifyId: "0DiWol3AO6WpXZgp0goxAV",
      youtubeId: "FGBhQbmPwH8",
      releaseYear: 2000,
      country: "France",
      genre: "Electronic"
    },
    {
      id: 39,
      title: "Get Lucky",
      artist: "Daft Punk",
      album: "Random Access Memories",
      spotifyId: "2Foc5Q5nqNiosCNqttzHof",
      youtubeId: "5NV6Rdv1a3I",
      releaseYear: 2013,
      country: "France",
      genre: "Electronic"
    },
  
    // Soul - USA
    {
      id: 40,
      title: "Respect",
      artist: "Aretha Franklin",
      album: "I Never Loved a Man",
      spotifyId: "7s25THrKz86DM225dOYwnr",
      youtubeId: "6FOUqQt3Kg0",
      releaseYear: 1967,
      country: "USA",
      genre: "Soul"
    },
    {
      id: 41,
      title: "Superstition",
      artist: "Stevie Wonder",
      album: "Talking Book",
      spotifyId: "1h2xVEoJORqrg71HocgqXd",
      youtubeId: "0CFuCYNx-1g",
      releaseYear: 1972,
      country: "USA",
      genre: "Soul"
    },
  
    // Jazz - USA
    {
      id: 42,
      title: "What a Wonderful World",
      artist: "Louis Armstrong",
      album: "What a Wonderful World",
      spotifyId: "2dLLR6qlu5UJ5gk0dKz0h3",
      youtubeId: "VqhCQZaH4Vs",
      releaseYear: 1967,
      country: "USA",
      genre: "Jazz"
    },
  
    // Funk - USA
    {
      id: 43,
      title: "Uptown Funk",
      artist: "Mark Ronson",
      album: "Uptown Special",
      spotifyId: "32OlwWuMpZ6b0aN2RZOeMS",
      youtubeId: "OPf0YbXqDm0",
      releaseYear: 2014,
      country: "USA",
      genre: "Funk"
    },
  
    // Disco - USA
    {
      id: 44,
      title: "Stayin' Alive",
      artist: "Bee Gees",
      album: "Saturday Night Fever",
      spotifyId: "3YRCqOhFifThpSRFJ1VWFM",
      youtubeId: "fNFzfwLM72c",
      releaseYear: 1977,
      country: "UK",
      genre: "Disco"
    },
  
    // Rap - France
    {
      id: 45,
      title: "Djadja",
      artist: "Aya Nakamura",
      album: "Nakamura",
      spotifyId: "1DgOYyW7OFjnD5rjm1cJxF",
      youtubeId: "iPGgnzc34tY",
      releaseYear: 2018,
      country: "France",
      genre: "Pop Rap"
    },
  
    // Indie - Iceland
    {
      id: 46,
      title: "Hoppípolla",
      artist: "Sigur Rós",
      album: "Takk...",
      spotifyId: "5W6lfIr1J1FJXPIh1zzUQW",
      youtubeId: "hnAwPeqrdAk",
      releaseYear: 2005,
      country: "Iceland",
      genre: "Indie"
    },
  
    // Pop - New Zealand
    {
      id: 47,
      title: "Royals",
      artist: "Lorde",
      album: "Pure Heroine",
      spotifyId: "2dLLR6qlu5UJ5gk0dKz0h3",
      youtubeId: "nlcIKh6sBtc",
      releaseYear: 2013,
      country: "New Zealand",
      genre: "Pop"
    },
  
    // Samba - Brazil
    {
      id: 48,
      title: "The Girl from Ipanema",
      artist: "Stan Getz & João Gilberto",
      album: "Getz/Gilberto",
      spotifyId: "5hslUAKq9I9CG2bAulFkHN",
      youtubeId: "RDsVdaFQhS86E",
      releaseYear: 1964,
      country: "Brazil",
      genre: "Bossa Nova"
    },
  
    // Metal - Finland
    {
      id: 49,
      title: "I Wish I Had an Angel",
      artist: "Nightwish",
      album: "Once",
      spotifyId: "4JLXHzz6hjHxP0iRCPyHw0",
      youtubeId: "OShcmfjO4rA",
      releaseYear: 2004,
      country: "Finland",
      genre: "Metal"
    },
  
    // Blues - USA
    {
      id: 50,
      title: "The Thrill Is Gone",
      artist: "B.B. King",
      album: "Completely Well",
      spotifyId: "7pgvJLSn7JQTRbHYuca5WQ",
      youtubeId: "4fk2prKnYnI",
      releaseYear: 1969,
      country: "USA",
      genre: "Blues"
    },
  
    // Gospel - USA
    {
      id: 51,
      title: "Oh Happy Day",
      artist: "Edwin Hawkins Singers",
      album: "Let Us Go Into the House of the Lord",
      spotifyId: "3mHD6eVq13vTALjUVQ9nMu",
      youtubeId: "KJohGa66FJM",
      releaseYear: 1968,
      country: "USA",
      genre: "Gospel"
    },
  
    // Indie Pop - Japan
    {
      id: 52,
      title: "Stay With Me",
      artist: "Miki Matsubara",
      album: "Pocket Park",
      spotifyId: "3kZCbJU7vMQoWfgJNiYZD4",
      youtubeId: "VEe_yIbW64w",
      releaseYear: 1979,
      country: "Japan",
      genre: "City Pop"
    }
  ];
  
  // Get song for today (deterministic based on date)
  export function getDailySong() {
    const today = new Date();
    const startDate = new Date('2024-01-01');
    const daysSinceStart = Math.floor((today - startDate) / (1000 * 60 * 60 * 24));
    const songIndex = daysSinceStart % SONG_DATABASE.length;
    return SONG_DATABASE[songIndex];
  }
  
  // Get random songs for search/autocomplete
  export function getRandomSongs(count = 20) {
    const shuffled = [...SONG_DATABASE].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, count);
  }